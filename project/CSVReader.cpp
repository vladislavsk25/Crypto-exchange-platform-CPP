...
{ 
    ...
    std::ifstream csvFile{csvfilename}; // provided csv file
    std::string line;
...
std::vector<std::string> CSVReader::tokenise(std::string csvLine, char separator) 
{
    ...
    return tokens;
        ...
        OrderBookEntry CSVReader::stringsToOBE(std::vector<std::string> tokens)
        {
            ...
            if (tokens.size() != 5)
            {
                cout << "Bad line " << std::endl;
                throw std::exception{};
            }

            try
            {
                price = std::stod(tokens[3]); //string to doubles
...
{               
            ...
            try
            {
                price = std::stod(priceString); //string to doubles
